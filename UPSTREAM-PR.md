...
app-limited-from-birth, below-target ramp). The `pub(crate)` widening of
`RttEstimator::new` they need landed upstream on `0.11.x` (commit
`33ce0c21`). The same buggy lines are present on upstream main, so the patch
should port trivially past the 0.11 line.
...
applies `git apply`-clean on the upstream `0.11.x` base (commit `33ce0c21`,
the released quinn-proto 0.11.16 state plus upstream's CUBIC saturation and
clippy follow-ups).
...
cd quinn && git checkout 33ce0c21 -b initial-fragmentation-control
