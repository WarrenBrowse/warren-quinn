...
impl Datagrams<'_> {
    ...
    pub fn send_classified(
        ...
        let Some(max_buffer_payload) = send_buffer_size.checked_sub(QUEUED_OVERHEAD) else {
        ...
        if drop {
            let evicted = self.conn.datagrams.make_space_for(data.len(), limit);
            self.conn.stats.datagram_tx.dropped_overflow += evicted;
        } else if !self.conn.datagrams.has_send_buffer_space(data.len(), limit) {
...
/// Per-entry bookkeeping charged against the send buffer alongside the payload
///
/// Payload-only accounting lets an application queue an unbounded number of
...
pub(super) struct DatagramState {
    ...
    /// Queued outgoing payload bytes, summed over every active flow
    pub(super) outgoing_total: usize,
    /// Queued outgoing datagrams, summed over every active flow
    ///
    /// Kept alongside `outgoing_total` so the send-buffer limit can charge
    /// [`QUEUED_OVERHEAD`] per entry without walking the flow map on the
    /// enqueue path.
...
impl DatagramState {
    ...
    /// Bytes the outgoing queue is charged for: payload plus per-entry
    /// bookkeeping
    fn outgoing_memory_used(&self) -> usize {
        self.outgoing_total
            .saturating_add(self.outgoing_count.saturating_mul(QUEUED_OVERHEAD))
    }
    ...
    /// Evict until a `datagram_len`-byte datagram fits under `send_buffer_size`
    ///
    /// Returns how many datagrams were dropped. An empty queue ends the loop:
    /// a limit smaller than one entry's overhead is reachable through the
    /// BDP adaptation, and must not panic the enqueue path.
    fn make_space_for(&mut self, datagram_len: usize, send_buffer_size: usize) -> u64 {
        let mut evicted = 0;
        while !self.has_send_buffer_space(datagram_len, send_buffer_size) {
            let Some(len) = self.evict_from_fattest_flow() else {
                break;
            };
    ...
    fn has_send_buffer_space(&self, datagram_len: usize, send_buffer_size: usize) -> bool {
        let Some(total) = self
            .outgoing_memory_used()
            .checked_add(datagram_len)
            .and_then(|total| total.checked_add(QUEUED_OVERHEAD))
        else {
            return false;
        };
    ...
    pub(super) fn drop_oversized(&mut self, max_payload: usize) -> bool {
        ...
        let outgoing_count = &mut self.outgoing_count;
...
mod tests {
    ...
    /// Queued datagrams across every flow, recounted from scratch.
    fn recount_entries(state: &DatagramState) -> usize {
        state.flows.values().map(|q| q.queue.len()).sum()
    }

    #[test]
    fn make_space_for_accounts_for_the_new_datagram() {
        let mut state = DatagramState::default();
        let now = Instant::now();
        push(&mut state, SHARED_BUCKET, 0xaa, 7, now);
        push(&mut state, SHARED_BUCKET, 0xbb, 2, now);

        assert_eq!(state.make_space_for(4, 10 + 2 * QUEUED_OVERHEAD), 1);

        assert_eq!(recount_entries(&state), 1);
        assert_eq!(state.outgoing_total, 2);
        assert_eq!(state.outgoing_count, 1);
    }
    ...
    fn make_space_for_handles_overflowing_capacity_check() {
        let mut state = DatagramState::default();
        let now = Instant::now();
        push(&mut state, SHARED_BUCKET, 0xaa, 1, now);
        // A byte count near the top of the range must not wrap the
        // `used + len + overhead` sum into a value that looks like room.
        state.outgoing_total = usize::MAX - 1;
        state.flows.get_mut(&SHARED_BUCKET).unwrap().bytes = usize::MAX - 1;

        assert_eq!(state.make_space_for(2, usize::MAX), 1);
    ...
    fn make_space_for_accounts_for_empty_datagram_metadata() {
        let mut state = DatagramState::default();
        let now = Instant::now();
        // Payload-only accounting would let this queue grow without bound:
        // every entry costs zero bytes and the window never fills.
        for _ in 0..2 {
            push(&mut state, SHARED_BUCKET, 0, 0, now);
        }
        let window = 2 * QUEUED_OVERHEAD;

        assert!(!state.has_send_buffer_space(0, window));
        assert_eq!(state.make_space_for(0, window), 1);
        assert_eq!(recount_entries(&state), 1);
        assert!(state.has_send_buffer_space(0, window));
        push(&mut state, SHARED_BUCKET, 0, 0, now);
        assert_eq!(state.outgoing_memory_used(), window);
    }
    ...
    fn drop_oversized_keeps_datagrams_at_limit() {
        let mut state = DatagramState::default();
        let now = Instant::now();
        push(&mut state, SHARED_BUCKET, 0xaa, 10, now);
        push(&mut state, SHARED_BUCKET, 0xbb, 11, now);
    ...
    #[test]
    fn outgoing_count_tracks_every_queue_mutation() {
        let mut state = DatagramState::default();
        let config = aqm();
        let mut stats = DatagramTxStats::default();
        let start = Instant::now();
        ...
        // One bucket, so the same CoDel state sees every visit.
        for tag in 0..8u8 {
            push(&mut state, SHARED_BUCKET, tag, 1200, start);
        }
        assert_eq!(state.outgoing_count, recount_entries(&state));

        // Transmit one.
        assert!(state.write(&mut buf, 65_535, start, None, &mut stats));
        assert_eq!(state.outgoing_count, recount_entries(&state));

        // Head-drop one: the first visit above target only arms the interval,
        // the visit after it elapses is the one that drops.
        let late = start + config.target * 4;
        state.write(&mut buf, 65_535, late, Some(&config), &mut stats);
        state.write(
            &mut buf,
            65_535,
            late + config.interval * 2,
            Some(&config),
            &mut stats,
        );
        assert!(stats.dropped_aqm > 0, "the AQM must have dropped a head");
        assert_eq!(state.outgoing_count, recount_entries(&state));

        // Prune by size, then evict for space.
        state.drop_oversized(600);
        assert_eq!(state.outgoing_count, recount_entries(&state));
        state.make_space_for(1200, 0);
        assert_eq!(state.outgoing_count, recount_entries(&state));
        assert_eq!(state.outgoing_count, 0);
        assert_eq!(state.outgoing_total, 0);
    }
